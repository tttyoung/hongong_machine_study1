...
## 과제: 아래 5가지 옵티마이저의 개념을 설명하고, 어떤 문제를 해결하기 위해, 어떤 아이디어를 적용했는지 작성. Momentu, Adagrad, RMSProp, Adam, AdamW

### 옵티마이저
손실함수의 최적해를 가장 빠르고 효율적으로 찾아가는 것<br>

#### SGD의 문제점
1.진동 문제: 골짜기가 길고 좁은 경우, 좌우로 심하게 왔다 갔다 하며 비효율적으로 내려갑니다.<br>
2.학습률 문제: 모든 파라미터(가중치)에 동일한 학습률(보폭)을 적용하는 것이 비효율적입니다.<br>

### Momentum
진동 문제 해결을 위해 과거에 이동했던 방향과 속도 고려<br>
과거 속도+현재 경사->다음 이동 방향 결정

### Adagrad
학습률 문제 해결을 위해 파리미터 별 변화값을 기록<br>
1.각 파라미터의 과거 경사(기울기) 값들을 제곱하여 계속 더해나갑니다.<br>
2.파라미터를 업데이트할 때, 학습률을 이 누적된 값의 제곱근으로 나눠줍니다.<br>

### RMSProp
Adagrad 학습 오래 진행->학습률 0에 수렴되는 문제 발생<br>
먼 과거 정보는 최신정보에 집중<br>
1.과거 모든 경사의 합이 아닌, '최근 N개의 경사' 정보만 가지고 분모를 계산합니다. (지수 이동 평균을 사용하여 구현)<br>
2.이것으로 학습률을 나눠줍니다.<br>

### Adam
Momentum + RMSProp<br>
1.모멘텀처럼 과거 경사들의 지수 이동 평균을 계산하여 이동할 방향(속도)을 관리합니다. <br>
2.RMSProp처럼 과거 경사들의 제곱값의 지수 이동 평균을 계산하여 파라미터별 학습률(보폭)을 관리합니다.<br>

### AdamW
Adam의 가중치 감소 방식을 개선한 버전<br>
Adam: (실제 경사 + 가중치 감소 항)을 계산한 뒤, 이 합쳐진 값에 Adam의 최적화 공식을 적용했습니다.<br>
AdamW: 실제 경사에만 Adam의 최적화 공식을 적용하여 파라미터 업데이트 값을 계산합니다. 그 후, 별도로 계산된 가중치 감소 값을 최종적으로 파라미터에서 빼줍니다.<br>
